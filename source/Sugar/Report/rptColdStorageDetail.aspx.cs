...
public partial class Sugar_Report_rptColdStorageDetail : System.Web.UI.Page
{
    int billno;
    int company_code;
    int year_code;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (isAuthenticate == "1" || User_Type == "A")
            {
                ...
                rpt.Load(Server.MapPath("CryColdStorageDetail.rpt"));
                rpt.SetDataSource(dt);

                CryColdStorageDetail.ReportSource = rpt;

                rpt.DataDefinition.FormulaFields["companyname"].Text = "\"" + company_name + "\"";
                rpt.DataDefinition.FormulaFields["fromto"].Text = "\"Partywise Cold Storage Detail report From " + datefrom + " To " + dateto + "\"";
                CryColdStorageDetail.RefreshReport();
    ...
    private DataTable GetData(string from, string to)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            if (ac_code != "0")
            {
                SqlCommand cmd = new SqlCommand("select * from qryColdStorageBalanceDetail where  doc_date between '" + from +
                    "' and '" + ToDt + "'", con);
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        //rprt1.Dispose();
        //GC.Collect();
        this.CryColdStorageDetail.ReportSource = null;

        CryColdStorageDetail.Dispose();
