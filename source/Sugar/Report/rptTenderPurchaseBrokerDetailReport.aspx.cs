...
public partial class Report_rptTenderPurchaseBrokerDetailReport : System.Web.UI.Page
{
    int billno;
    int company_code;
    int year_code;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (isAuthenticate == "1" || User_Type == "A")
            {
                ...
                rpt.Load(Server.MapPath("cryTrnderPSBrokerReportDetails.rpt"));
                rpt.SetDataSource(dt);

                cryTrnderPSBrokerReportDetails.ReportSource = rpt;
                ...
                cryTrnderPSBrokerReportDetails.RefreshReport();
    ...
    private DataTable GetData(string from, string to)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            if (ac_code != "0")
            {
                SqlCommand cmd = new SqlCommand("select * from qrytenderheaddetail where   Buyer_Party='" + ac_code + "' and Tender_Date between '" + datefrom.ToString() +
            ...
            {
                SqlCommand cmd = new SqlCommand("select * from qrytenderheaddetail where  Buyer_Party!='2' and Tender_Date between '" + datefrom.ToString() +
                    "' and '" + dateto.ToString() + "'", con);
                cmd.CommandType = CommandType.Text;
                SqlDataAdapter sda = new SqlDataAdapter(cmd);
                sda.Fill(dt);
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        //rprt1.Dispose();
        //GC.Collect();
        this.cryTrnderPSBrokerReportDetails.ReportSource = null;

        cryTrnderPSBrokerReportDetails.Dispose();
