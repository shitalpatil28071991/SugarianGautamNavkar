...
public partial class Report_rptDetailSellBrokerReport : System.Web.UI.Page
{
    ...
    private DataTable GetData(string from, string to)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            if (ac_code != "0")
            {
                SqlCommand cmd = new SqlCommand("select * from qryRetailSale where Broker_Code='" + ac_code + "' and doc_date between '" + FromDt.ToString() +
            ...
            {
                SqlCommand cmd = new SqlCommand("select * from qryRetailSale where Broker_Code!='0' and doc_date between '" + FromDt.ToString() +
              "' and '" + ToDt.ToString() + "' and Year_Code='" + Convert.ToInt32(Session["year"].ToString()) + "'", con);
                //SqlCommand cmd = new SqlCommand("select * from NT_1_qrySugarRetailSellList where doc_no='" + ac_code + "' and doc_date between '" + datefrom.ToString() +
                //    "' and '" + dateto.ToString() + "'", con);
                cmd.CommandType = CommandType.Text;
