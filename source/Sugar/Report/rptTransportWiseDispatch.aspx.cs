...
//              ",mill_rate as MR,quantal as Qntl,sale_rate as SR,truck_no as lorry,FreightPerQtl as frt,vasuli_rate1 AS vasuli,transportname as transport," +
//              " doname as do,purc_no as tender,LoadingSms,transport as trancode from qrydohead  where company_code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and Year_Code=" + Convert.ToInt32(Session["year"].ToString()) + " and purc_no!=0 and doc_date between '" + fromDT + "' and '" + toDT + "' and tran_type='DO'";
            ...
            qry = "SELECT     dbo.nt_1_deliveryorder.doc_no, CONVERT(varchar(10), dbo.nt_1_deliveryorder.doc_date, 103)  AS dodate, mill.Short_Name AS millShortName, vb.Short_Name AS VocBy, gp.Short_Name AS GetPass, "
                      ...
                      + " dbo.nt_1_deliveryorder.purc_no AS tender, vasuli.Ac_Name_E AS vasuliacname, dbo.nt_1_deliveryorder.LoadingSms,dbo.nt_1_deliveryorder.MM_Rate, transport as trancode  "
...
//            qry = "select distinct(transport) as TransportCode,TransportName as TransportName from qrydohead where tran_type='DO' and company_code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and Year_Code=" + Convert.ToInt32(Session["year"].ToString()) + " and  doc_date between '" + fromDT + "' and '" + toDT + "' order by TransportName asc";

            qry = "select distinct(dbo.nt_1_deliveryorder.transport) as TransportCode , dbo.nt_1_accountmaster.Ac_Name_E  as TransportName  "
                  + " FROM         dbo.nt_1_deliveryorder LEFT OUTER JOIN "
                  + " dbo.nt_1_accountmaster ON dbo.nt_1_deliveryorder.tc = dbo.nt_1_accountmaster.accoid"
    ...
    protected void dtl_OnItemDataBound(object sender, DataListItemEventArgs e)
    {
        try
        {
            ...
            DataView view = new DataView(SecoundQry, "trancode='" + lblMillCode.Text + "'", "trancode", DataViewRowState.CurrentRows);
