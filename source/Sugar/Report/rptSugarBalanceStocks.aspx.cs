...
public partial class Reports_rptSugarBalanceStocks : System.Web.UI.Page
{
    ...
    private void Bindata()
    {
        try
        {
            ...
            string qry = "select distinct Tender_No,Tender_DateConverted as Tender_Date,millshortname as millname,season,Grade,Quantal as Quantal" +
                      ",Mill_Rate as Mill_Rate ,Purc_Rate as Purc_Rate,Lifting_DateConverted as Lifting_Date,tenderdoshortname as doname " +
                      ...
                      " where BALANCE!=0 and dbo.qrytenderhead.Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " order by millshortname,Tender_No";
            ...
            string qry1 = "select ID as ID,Buyer,qrybuyer.Ac_Name_E AS buyerbrokerfullname,Sale_Rate as Sale_Rate,Buyer_Quantal,DESPATCH as despatchqty,BALANCE as balance,Tender_No,loding_by_us " +
                          " FROM dbo.qrytenderdobalanceviewraw LEFT OUTER JOIN dbo.qrymstaccountmaster AS qrybuyer ON dbo.qrytenderdobalanceviewraw.buyerid = qrybuyer.accoid LEFT OUTER JOIN " +
                          " dbo.qrytenderhead ON dbo.qrytenderdobalanceviewraw.tenderid = dbo.qrytenderhead.tenderid where dbo.qrytenderhead.Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " order by ID ";
            ...
            if (dtAcData.Rows.Count > 0)
            {
                ...
                if (dtAcData.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dtAcData.Rows.Count; i++)
                    {
                        ...
                        dr["balance"] = balance;
                        dr["loding_by_us"] = dtAcData.Rows[i]["loding_by_us"].ToString();
                        string loding_by_us = dtAcData.Rows[i]["loding_by_us"].ToString();

                        if (loding_by_us == "Y")
                        {
                            loding_by_us = "Loading By Us";
                        }
                        else
                        {
                            loding_by_us = "";
                        }
                        dr["loding_by_us"] = loding_by_us;
                        if (balance != 0)
                        {
                            dt.Rows.Add(dr);
                        }
