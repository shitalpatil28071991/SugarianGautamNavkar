...
public partial class Sugar_Report_rptsalemonwise : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (isAuthenticate == "1" || User_Type == "A")
            {
                ...
                pagehead = "Sale Qty Montwise from " + fromdt + " To " + todt;
                if (!IsPostBack)
                {
                ...
                rprt1.Load(Server.MapPath("Crysalemonwise.rpt"));
                rprt1.SetDataSource(dt);
                Crysalemonwise.ReportSource = rprt1;
    ...
    private DataTable GetData(string fromdt, string todt, string accode)
    {
        ...
        {
            SqlCommand cmd;
            //cmd = new SqlCommand("select * from qryMonthwisesale where doc_date  between '" + datefrom + "' and '" + dateto
            //        + "' and  Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and Year_Code=" + Convert.ToInt32(Session["year"].ToString()), con);
            cmd = new SqlCommand("select YEAR(dbo.nt_1_sugarsale.doc_date) AS yr, MONTH(dbo.nt_1_sugarsale.doc_date) AS mn, dbo.nt_1_sugarsale.Company_Code, dbo.nt_1_sugarsale.Year_Code, "+
                      "ISNULL(SUM(dbo.nt_1_sugarsaledetails.Quantal), 0) AS qntl, ISNULL(SUM(dbo.nt_1_sugarsaledetails.item_Amount), 0) AS itmamount FROM dbo.nt_1_sugarsale INNER JOIN "+
                      " dbo.nt_1_sugarsaledetails ON dbo.nt_1_sugarsale.saleid = dbo.nt_1_sugarsaledetails.saleid "+
                      " where  dbo.nt_1_sugarsale.IsDeleted!=0 and dbo.nt_1_sugarsale.doc_date  between '" + datefrom + "' and '" + dateto + "' and  nt_1_sugarsale.Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString())
                      + " and nt_1_sugarsale.Year_Code=" + Convert.ToInt32(Session["year"].ToString())+" "+
                      " GROUP BY dbo.nt_1_sugarsale.Company_Code, dbo.nt_1_sugarsale.Year_Code, YEAR(dbo.nt_1_sugarsale.doc_date), MONTH(dbo.nt_1_sugarsale.doc_date)",con);
    ...
    protected void btnPDF_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string filename = filepath + "\\Salemonthwise" + company_code + "_" + year_code + "_" + DateTime.Now.ToString("ddMMyyyy_HHmmss") + ".pdf";
    ...
    protected void btnMail_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (txtEmail.Text != string.Empty)
            {
                ...
                contentType.Name = "Salemonthwise";
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        this.Crysalemonwise.ReportSource = null;

        Crysalemonwise.Dispose();
