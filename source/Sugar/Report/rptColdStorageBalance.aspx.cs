...
public partial class Sugar_Report_rptColdStorageBalance : System.Web.UI.Page
{
    int billno;
    int company_code;
    int year_code;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (isAuthenticate == "1" || User_Type == "A")
            {
                ...
                rpt.Load(Server.MapPath("CryColdStorageBalance.rpt"));
                rpt.SetDataSource(dt);

                CryColdStorageBalance.ReportSource = rpt;
                rpt.DataDefinition.FormulaFields["fromto"].Text = "\"Cold Storage Balance report From " + datefrom + " To " + dateto + "\"";
                ...
                CryColdStorageBalance.RefreshReport();
    ...
    private DataTable GetData(string from, string to)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            if (ac_code != "0")
            {
                SqlCommand cmd = new SqlCommand("select * from qryColdStorageBalance where  doc_date between '" + FromDt +
            ...
            {
                SqlCommand cmd = new SqlCommand("select * from qryColdStorageBalance where  doc_date between '" + from +
                    "' and '" + ToDt + "'", con);
                cmd.CommandType = CommandType.Text;
                SqlDataAdapter sda = new SqlDataAdapter(cmd);
                sda.Fill(dt);
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        //rprt1.Dispose();
        //GC.Collect();
        this.CryColdStorageBalance.ReportSource = null;

        CryColdStorageBalance.Dispose();
