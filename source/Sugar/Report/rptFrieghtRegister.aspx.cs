...
public partial class Report_rptFrieghtRegister : System.Web.UI.Page
{
    ...
    protected void DataList_ItemDataBound(object sender, DataListItemEventArgs e)
    {
        ...
        if (string.IsNullOrEmpty(Branch_Code))
        {
            ...
            " left outer join " + tblPrefix + "AccountMaster b on d.mill_code=b.Ac_Code AND d.company_code=b.Company_Code left outer join " + tblPrefix + "AccountMaster c on d.transport=c.Ac_Code AND d.company_code=c.Company_Code  where d.company_code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and d.Year_Code=" + Convert.ToInt32(Session["year"].ToString()) + " and tran_type='DO' and d.doc_date='" + date + "' and d.purc_no!=0";
        }
        else
        {
            qry = "select d.doc_no as Memo_No,a.Ac_Name_E as Party,b.Short_Name as mill,d.quantal,d.truck_no as Veh_No,Freight_Amount as frieghtAmt,ISNULL((d.Memo_Advance-d.vasuli_amount),0) as Advance,d.FreightPerQtl as frieght,(c.Ac_Name_E + ' (' + CAST(c.Ac_Code as NVARCHAR(max))+')') as Transport" +
            " from " + tblPrefix + "deliveryorder d  left outer join " + tblPrefix + "AccountMaster a on d.GETPASSCODE=a.Ac_Code AND d.company_code=a.Company_Code" +
            " left outer join " + tblPrefix + "AccountMaster b on d.mill_code=b.Ac_Code AND d.company_code=b.Company_Code left outer join " + tblPrefix + "AccountMaster c on d.transport=c.Ac_Code AND d.company_code=c.Company_Code  where d.company_code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and d.Year_Code=" + Convert.ToInt32(Session["year"].ToString()) + " and d.Branch_Code=" + Branch_Code + " and tran_type='DO' and d.doc_date='" + date + "' and d.purc_no!=0";
        }
        ds = new DataSet();
        ds = clsDAL.SimpleQuery(qry);
        if (ds != null)
