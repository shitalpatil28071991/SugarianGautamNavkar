...
public partial class Sugar_Report_rptpurchasetds1 : System.Web.UI.Page
{
    ...
    private DataTable GetData(string fromdt, string todt, string accode)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            if (Accode == "0")
            {
                cmd = new SqlCommand("select * from qryTCSPurchaseUnion where date between '" + datefrom + "' and '" + dateto
                    + "'  and TDS_Amt !=0 and Year_Code=" + Convert.ToInt32(Session["year"].ToString()) + "  and Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " order by tran_type,doc_no", con);
            }
            else
            {
                cmd = new SqlCommand("select * from qryTCSPurchaseUnion where date  between '" + datefrom + "' and '" + dateto
                    + "' and TDS_Amt !=0 and Year_Code=" + Convert.ToInt32(Session["year"].ToString()) + " and Party_Code= '" + Accode + "' and  Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " order by tran_type,doc_no", con);
            }
