    ...
    private void bindData()
    {
        ...
        try
        {
            using (clsDataProvider obj = new clsDataProvider())
            {
                ...
                qry = "select TRAN_TYPE,DOC_NO,Convert(varchar(10),DOC_DATE,102) as DOC_DATE ,NARRATION,AMOUNT,ADJUSTED_AMOUNT,AC_CODE,UNIT_Code,DRCR,SORT_TYPE,SORT_NO,'' as DO_NO from " + tblGLEDGER +
                        " where AC_CODE=" + accode + " and DOC_DATE between '" + fromdt + "' and '" + todt + "' and  Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " order by DOC_DATE asc,tran_type,cashcredit,doc_no,SORT_TYPE,SORT_NO,ORDER_CODE ";
    ...
    protected void lnkGo_Click(object sender, EventArgs e)
    {
        ...
        if (No != "0")
        {
            if (TranType.Contains("BP") || TranType.Contains("CR") || TranType.Contains("CP") || TranType.Contains("BR"))
            {
                ...
                {
                    Int32 action = 1;
                    qryCommon="select tranid from nt_1_transacthead where doc_no='" + No + "' and tran_type='" + sessionName + "' and Company_Code='" + Convert.ToInt32(Session["Company_Code"]).ToString() + "' and Year_Code='" + Convert.ToInt32(Session["year"]).ToString() + "'";
            ...
            else
            {
                ...
                else if (dt1 > dt2xml)
                {
                    ...
                    else if (sessionName == "GT_No")
                    {
                        Int32 action = 1;
                        Int32 count = Convert.ToInt32(clsCommon.getString("select grouptenderid from GroupTenderPurchase where Tender_No='" + No + "'   and Company_Code='" + Convert.ToInt32(Session["Company_Code"]).ToString() + "' and Year_Code='" + Convert.ToInt32(Session["year"]).ToString() + "'"));
                        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), TranType, "javascript:GTxml('" + count + "','" + action + "');", true);
                    }
                    else
                    {
                        Session[sessionName] = No;
