...
public partial class Report_rptTenderPurchaseBrokerReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (isAuthenticate == "1" || User_Type == "A")
            {
                ...
                rpt.Load(Server.MapPath("cryTrnderPSBrokerReport.rpt"));
                rpt.SetDataSource(dt);

                cryTrnderPSBrokerReport.ReportSource = rpt;
                ...
                cryTrnderPSBrokerReport.RefreshReport();
    ...
    private DataTable GetData(string from, string to)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            if (ac_code != "0")
            {
                SqlCommand cmd = new SqlCommand("select distinct Tender_No,Tender_Date,millname,paymenttoname,Quantal,brokername,Broker,Brokrage,millshortname,Tender_DateConverted from qrytenderheaddetail where Broker='" + ac_code + "' and Tender_Date between '" + datefrom.ToString() +
                   "' and '" + dateto.ToString() + "'", con);
            ...
            {
                SqlCommand cmd = new SqlCommand("select distinct Tender_No,Tender_Date,millname,paymenttoname,Quantal,brokername,Broker,Brokrage,millshortname,Tender_DateConverted from qrytenderheaddetail where Broker!='2' and Tender_Date between '" + datefrom.ToString() +
                   "' and '" + dateto.ToString() + "'", con);
                cmd.CommandType = CommandType.Text;
                SqlDataAdapter sda = new SqlDataAdapter(cmd);
                sda.Fill(dt);
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        //rprt1.Dispose();
        //GC.Collect();
        this.cryTrnderPSBrokerReport.ReportSource = null;

        cryTrnderPSBrokerReport.Dispose();
