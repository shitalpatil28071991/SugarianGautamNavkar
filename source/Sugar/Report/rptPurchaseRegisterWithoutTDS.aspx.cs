...
using System.Data;
public partial class Sugar_Report_rptPurchaseRegisterWithoutTDS : System.Web.UI.Page
{
    ...
    private void BindList()
    {
        try
        {
            ...
            if (Ac_Code == string.Empty || Ac_Code == "0")
            {
            ...
            {
                qry = "Select distinct(CONVERT(VARCHAR(10),DOC_DATE,102)) as DOC_DATE from NT_1_qryAwakPurchase where Ac_Code=" + Ac_Code + " and DOC_DATE between'" + fromDTnew + "' and '" + toDTnew + "' and Company_Code="
                   + Convert.ToInt32(Session["Company_Code"].ToString()) + " and [Year_Code]=" + Convert.ToInt32(Session["year"].ToString()) + " order by DOC_DATE";
            }
            ds = new DataSet();
    ...
    protected void dtl_OnItemDataBound(object sender, DataListItemEventArgs e)
    {
        try
        {
            using (clsDataProvider obj = new clsDataProvider())
            {
                ...
                if (Ac_Code == string.Empty || Ac_Code == "0")
                {
                    qry = "select distinct doc_no,CONVERT(VARCHAR(10),DOC_DATE,103) as DOC_DATE,Ac_Code,Supplier,netqty,purchasevalue,MARKETSES,supercost,levihead,adat,tdsamount,AMOUNT,"
                ...
                {
                    qry = "select distinct doc_no,CONVERT(VARCHAR(10),DOC_DATE,103) as DOC_DATE,Ac_Code,Supplier,netqty,purchasevalue,MARKETSES,supercost,levihead,adat,tdsamount,AMOUNT,"
                       + " convert(DECIMAL(10,2),round((AMOUNT)-(tdsamount),2)) as payable,billno from NT_1_qryAwakPurchase where Ac_Code=" + Ac_Code + " and Company_Code="
                       + Convert.ToInt32(Session["Company_Code"].ToString()) + " and Year_Code=" + Convert.ToInt32(Session["year"].ToString())
                       + " and DOC_DATE='" + date + "'";
                }
