...
public partial class Report_rptTenderPurchaseBrokerReport : System.Web.UI.Page
{
    int billno;
    int company_code;
    int year_code;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (isAuthenticate == "1" || User_Type == "A")
            {
                ...
                rpt.Load(Server.MapPath("cryTenderDOReport.rpt"));
                rpt.SetDataSource(dt);

                cryTenderDOReport.ReportSource = rpt;
                ...
                cryTenderDOReport.RefreshReport();
    ...
    private DataTable GetData(string from, string to)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            //SqlCommand cmd = new SqlCommand("select * from NT_1_qryTenderListReport where  Broker is not null and Broker='" + ac_code + "' and Tender_Date between '" + datefrom.ToString() +
            //    "' and '" + dateto.ToString() + "'", con);
            if (ac_code != "0")
            {
                SqlCommand cmd = new SqlCommand("select distinct Tender_No,Tender_Date,millshortname,paymenttoname,Quantal,tenderdoname,Tender_Do from qrytenderheaddetail where Tender_Do='" + ac_code + "' and Tender_Date between '" + datefrom.ToString() +
                   "' and '" + dateto.ToString() + "'", con);
            ...
            {
                SqlCommand cmd = new SqlCommand("select distinct Tender_No,Tender_Date,millshortname,paymenttoname,Quantal,tenderdoname,Tender_Do from qrytenderheaddetail where Tender_Do!='2' and Tender_Date between '" + datefrom.ToString() +
                   "' and '" + dateto.ToString() + "'", con);
                cmd.CommandType = CommandType.Text;
                SqlDataAdapter sda = new SqlDataAdapter(cmd);
                sda.Fill(dt);
            ...
            //if (ac_code != "0")
            //{
            //    SqlCommand cmd = new SqlCommand("select * from NT_1_qryTenderList where Broker is not null and UtrDate between '" + from +
            ...
            //else
            //{
            //    SqlCommand cmd = new SqlCommand("select * from NT_1_qryDOUTRNewReport where LTNo is not null and UtrDate between '" + from +
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        //rprt1.Dispose();
        //GC.Collect();
        this.cryTenderDOReport.ReportSource = null;

        cryTenderDOReport.Dispose();
