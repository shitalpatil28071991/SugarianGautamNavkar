...
public partial class Report_rptJaggerySaudaProfit : System.Web.UI.Page
{
    ...
    private void BindList()
    {
        try
        {
            ...
            qry = "Select distinct convert(varchar(10),[DOC_DATE],102) as doc_date  from " + tblPrefix + "qryJaggerySaudaProfit where DOC_DATE between'" + fromDTnew
                + "' and '" + toDTnew + "' and Company_Code="
                + Convert.ToInt32(Session["Company_Code"].ToString()) + " and [Year_Code]=" + Convert.ToInt32(Session["year"].ToString()) + " order by doc_date";
            ds = new DataSet();
    ...
    protected void dtl_OnItemDataBound(object sender, DataListItemEventArgs e)
    {
        try
        {
            using (clsDataProvider obj = new clsDataProvider())
            {
                ...
                qry = "select [Tran_Type],[Doc_No],[Bill_No],[Qty],[Net_Wt],[Purc_Rate],[Purc_Amnt],[Sale_Rate],[Sale_Amnt],Abs([Loss]) as [Loss],convert(varchar(10),[DOC_DATE],103) as doc_date,[Ac_Name_E],[System_Name_E]" +
                  " from " + tblPrefix + "qryJaggerySaudaProfit where Company_Code="
                  + Convert.ToInt32(Session["Company_Code"].ToString()) + " and Year_Code=" + Convert.ToInt32(Session["year"].ToString())
                  + " and DOC_DATE='" + date + "'";
