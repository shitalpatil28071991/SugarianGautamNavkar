...
public partial class Sugar_Report_rptpurcmonwise : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (isAuthenticate == "1" || User_Type == "A")
            {
                ...
                pagehead = "Purchase Qty Montwise from " + fromdt + " To " + todt;
                if (!IsPostBack)
                {
                ...
                rprt1.Load(Server.MapPath("Crypurcmonwise.rpt"));
                rprt1.SetDataSource(dt);
                Crypurcmonwise.ReportSource = rprt1;
    ...
    private DataTable GetData(string fromdt, string todt, string accode)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            cmd = new SqlCommand("SELECT dbo.nt_1_sugarpurchase.Company_Code, dbo.nt_1_sugarpurchase.Year_Code, YEAR(dbo.nt_1_sugarpurchase.doc_date) AS yr,"+
                " MONTH(dbo.nt_1_sugarpurchase.doc_date) AS mn,ISNULL(SUM(dbo.nt_1_sugarpurchasedetails.Quantal), 0) AS qntl,"+
                " ISNULL(SUM(dbo.nt_1_sugarpurchasedetails.item_Amount), 0) AS itemamt FROM dbo.nt_1_sugarpurchase INNER JOIN dbo.nt_1_sugarpurchasedetails ON "+
                " dbo.nt_1_sugarpurchase.purchaseid = dbo.nt_1_sugarpurchasedetails.purchaseid "+
                " where dbo.nt_1_sugarpurchase.doc_date  between '" + datefrom + "' and '" + dateto + "' and  dbo.nt_1_sugarpurchase.Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString())
                + " and dbo.nt_1_sugarpurchase.Year_Code=" + Convert.ToInt32(Session["year"].ToString()) + " " +
                " GROUP BY dbo.nt_1_sugarpurchase.Company_Code, dbo.nt_1_sugarpurchase.Year_Code, YEAR(dbo.nt_1_sugarpurchase.doc_date), MONTH(dbo.nt_1_sugarpurchase.doc_date)", con);
    ...
    protected void btnPDF_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string filename = filepath + "\\purcmonwise" + company_code + "_" + year_code + "_" + DateTime.Now.ToString("ddMMyyyy_HHmmss") + ".pdf";
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        this.Crypurcmonwise.ReportSource = null;

        Crypurcmonwise.Dispose();
