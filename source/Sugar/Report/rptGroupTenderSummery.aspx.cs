...
public partial class Sugar_Report_rptGroupTenderSummery : System.Web.UI.Page
{
    int company_code;
    int year_code;
    ...
    private DataTable GetData()
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            SqlCommand cmd = new SqlCommand("select Tender_No ,GroupName,Tender_dateConverted,Lifting_DateConverted,Grade,Mill_Rate,Quantal, millName,MillDeff, membername, sum(Buyer_Quantal) as Buyer_Quantal,sum(ItemAmount ) as ItemAmount,sum(profit) as profit , " +
                                            " sum(indivisulProfit) as indivisulProfit,sum(millContribution) as millContribution,sum(paidorreseive) as paidorreseive,sum(paid) as paid,sum(otherPaid) as otherPaid,sum(profit) as profit  from qryGroupTenderHeadDetail " +
                                            " where Tender_No='" + Tender_No + "' and Company_Code= '" + company_code + "' and Year_Code= '" + year_code + "' and  membercode!=2" +
