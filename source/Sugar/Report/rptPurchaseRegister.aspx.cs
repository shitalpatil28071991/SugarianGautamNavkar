...
public partial class Sugar_Report_rptPurchaseRegister : System.Web.UI.Page
{
    ...
    private DataTable GetData(string fromdt, string todt)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            if (Accode != "0")
            {
                SqlCommand cmd = new SqlCommand("select * from qrypurchasehead where doc_date between '" + datefrom + "' and '" + dateto + "'and Ac_Code='" + Accode
            ...
            else
            {
                ...
                SqlCommand cmd = new SqlCommand("select * from qrypurchasehead where doc_date between '" + datefrom + "' and '" + dateto
                    + "' and  Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()), con);
                cmd.CommandType = CommandType.Text;
                SqlDataAdapter sda = new SqlDataAdapter(cmd);
                sda.Fill(dt);
