...
public partial class Sugar_Report_rptSBNo : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (isAuthenticate == "1" || User_Type == "A")
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        TotalAmount = dt.Rows[i]["TCS_Net_Payable"].ToString();

                        inWords = clsNoToWord.ctgword(TotalAmount);
                        dt.Rows[i]["Amount"] = inWords;
                        //Add carporate logic
                        string corporatenumber = dt.Rows[i]["Carporate_Sale_No"].ToString();
                ...
                rprt1.Load(Server.MapPath("CrySBNO_Register.rpt"));
                rprt1.SetDataSource(dt);
                CrySBNO_Register.ReportSource = rprt1;
              ...
              //  rprt1.DataDefinition.FormulaFields["word"].Text = "\"" + inWords + "\"";
    ...
    private DataTable GetData(string fromdt, string todt)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            {
                SqlCommand cmd = new SqlCommand("select * from qrysalebill where  doc_date between '" + datefrom + "' and '" + dateto + "'and company_code = " + Convert.ToInt32(Session["Company_Code"].ToString()) + " and Year_Code = " + Convert.ToInt32(Session["year"].ToString()) + "", con);
            ...
            {
                SqlCommand cmd = new SqlCommand("select * from qrysalebill where Ac_Code = " + Accode + " and  doc_date between '" + datefrom + "' and '" + dateto + "'and company_code = " + Convert.ToInt32(Session["Company_Code"].ToString()) + " and Year_Code = " + Convert.ToInt32(Session["year"].ToString()) + "", con);
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        //rprt1.Dispose();
        //GC.Collect();
        this.CrySBNO_Register.ReportSource = null;

        CrySBNO_Register.Dispose();
