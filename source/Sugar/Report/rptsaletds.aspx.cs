...
public partial class Sugar_Report_rptsaletds : System.Web.UI.Page
{
    ...
    private DataTable GetData(string fromdt, string todt, string accode)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            //        + "'  and IsDeleted!=0 and TDS_Amt !=0 and  tran_type in ('DN','DS') and Year_Code=" + Convert.ToInt32(Session["year"].ToString()) + " and  Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()), con);
            //}
            if (Accode == "0")
            {
                cmd = new SqlCommand("select * from qryTCSSaleUnion where Inv_date between '" + datefrom + "' and '" + dateto
