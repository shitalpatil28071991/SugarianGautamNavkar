        ...
        try
        {
            ...
            if (Broker_Code != string.Empty)
            {
                qry = "select Convert(varchar(10),u.doc_date,103) as dt,u.Tran_Type as ttype,u.doc_no as #,u.Unit_Name as Party,u.lorry as lorry,u.NETQNTL as Qntl,a.Short_Name as Mill,u.Bill_Amount as VocAmount," +
                    ...
                    " (u.Bill_Amount-ISNULL((Select SUM(t.Value) as Recieved from multiple_receipt_detail  t where t.Tran_Type IN('BR','CR','AB','CN','DN','RS','RR') and t.Tran_Type=u.Tran_Type and t.Bill_No = u.doc_no and t.Bill_Auto_Id = u.saleid and t.Year_Code=u.Year_Code),0)) as Short,u.BROKER,u.ASN,u.Year_Code from " + tblPrefix + "qryVoucherSaleUnion1 u" +
                    ...
                    "  and u.doc_date between '" + from + "' and '" + to + 
                    "' order by u.doc_date";
            ...
            if (ds != null)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        double Short1 = 0.00;
                        double totalrecieved = 0.00;
                        int Bill_YearCode = Convert.ToInt32(ds.Tables[0].Rows[i]["Year_Code"].ToString());
                        double voucAmount = Convert.ToDouble(ds.Tables[0].Rows[i]["VocAmount"].ToString());
                        qry = " select #,Ptype,tran_date,PayAmt,narration from " + tblPrefix + "qryLedgerShortPayment where Voucher_No=" + doc_no + " and Voucher_Type='" + tran_type + "' and Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and BillYearCode=" + Bill_YearCode + "";
                        ds2 = new DataSet();
                        ...
                        if (ds2 != null && ds2.Tables.Count > 0 && ds2.Tables[0].Rows.Count > 0)
                        {
                            ...
                            if (dt2.Rows.Count > 0)
                            {
                                ...
                                totalrecieved = Convert.ToDouble(dt2.Compute("SUM(PayAmt)", string.Empty));
                                Short1 = Math.Round((voucAmount - totalrecieved), 2);
                            }
                            else {
                                Short1 = 0;
                            }
                        ...
                        double VocAmount = Convert.ToDouble(ds.Tables[0].Rows[i]["VocAmount"].ToString());
                        double amt = VocAmount - totalrecieved;
                        if (amt != 0.00)
                        {
                            dt.Rows.Add(dr);
                        } 
