...
public partial class Report_rptSaleRegisterDateWise : System.Web.UI.Page
{
    ...
    private DataTable GetData(string fromdt, string todt)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            if (Accode != "0")
            {
                SqlCommand cmd = new SqlCommand("select * from qrysalehead where doc_date between '" + datefrom + "' and '" + dateto
                    + "' and Ac_Code='" + Accode + "' and IsDeleted!=0 and doc_no!=0 and  Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()), con);
            ...
            {
                SqlCommand cmd = new SqlCommand("select * from qrysalehead where doc_date between '" + datefrom + "' and '" + dateto
                    + "' and IsDeleted!=0 and doc_no!=0 and  Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()), con);
                cmd.CommandType = CommandType.Text;
                SqlDataAdapter sda = new SqlDataAdapter(cmd);
                sda.Fill(dt);
            }
