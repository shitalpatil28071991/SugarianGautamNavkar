...
public partial class Sugar_Report_rptSaleBillBalance : System.Web.UI.Page
{
    ...
    private DataTable GetData(string fromdt, string todt)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            if ( Accode!="0")
            {
                cmd = new SqlCommand("select * from qrysalebillbalance where Ac_Code=" + Accode + " and doc_date between '" + datefrom + "' and '" + dateto + "' and balance!=0 and Company_Code='"
               + Convert.ToInt32(Session["Company_Code"].ToString()) + "'", con);
            }
            else
            {
                cmd = new SqlCommand("select * from qrysalebillbalance where doc_date between '" + datefrom + "' and '" + dateto + "' and Ac_Code!=1 and balance!=0 and Company_Code='"
               + Convert.ToInt32(Session["Company_Code"].ToString()) + "'", con);
