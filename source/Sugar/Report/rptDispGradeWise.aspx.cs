...
public partial class Sugar_Report_rptDispGradeWise : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (isAuthenticate == "1" || User_Type == "A")
            {
                ...
                Accode = Request.QueryString["Tender_No"];
                ...
                rprt1.Load(Server.MapPath("CryDispGradeWise.rpt"));
                rprt1.SetDataSource(dt);
                CryDispGradeWise.ReportSource = rprt1;
    ...
    //        if (Accode != "0")
    //        {
    //            cmd = new SqlCommand("select * from qrydohead where Ac_Code=" + Accode + " and doc_date between '" + datefrom + "' and '" + dateto + "'", con);
    //        }
    //        else
    //        {
    //            cmd = new SqlCommand("select * from qrydohead where doc_date between '" + datefrom + "' and '" + dateto + "'", con);
    ...
    private DataTable GetData()
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            if (Accode != "0")
            {
                //cmd = new SqlCommand("select * from qrysalehead where Company_Code=" + company_code + " and Year_Code=" + year_code + "", con);
                cmd = new SqlCommand("select * from qrysalehead where doc_date between '" + datefrom + "' and '" + dateto + "' and Company_Code=" + company_code + " and Year_Code=" + year_code + "", con);
                cmd.CommandType = CommandType.Text;
                SqlDataAdapter sda = new SqlDataAdapter(cmd);
                sda.Fill(dt);
            }
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        //rprt1.Dispose();
        //GC.Collect();
        this.CryDispGradeWise.ReportSource = null;

        CryDispGradeWise.Dispose();
