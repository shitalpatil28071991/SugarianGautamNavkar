...
public partial class Sugar_Report_rptTDSReport : System.Web.UI.Page
{
    ...
    private void BindList()
    {
        try
        {
            ...
            {
                qry = "Select AC_CODE,supplier,SUM(purchasevalue) as Item_vaue,SUM(tdsamount)as tds,SUM(adat)as adat " +
               "from NT_1_qrypurchreport where Tran_Type='AW' and DOC_DATE between'" + fromDTnew + "' and '" + toDTnew + "' and " +
                "Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and [Year_Code]=" + Convert.ToInt32(Session["year"].ToString()) + " group by supplier,AC_CODE";
            }
            else
            {
                qry = "Select AC_CODE,supplier,SUM(purchasevalue) as Item_vaue,SUM(tdsamount)as tds,SUM(adat)as adat " +
           "from NT_1_qrypurchreport where tdsperc=" + tdsrate + " and Tran_Type='AW' and DOC_DATE between'" + fromDTnew + "' and '" + toDTnew + "' and " +
            "Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and [Year_Code]=" + Convert.ToInt32(Session["year"].ToString()) + " group by supplier,AC_CODE";
            }
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    purchaseval = Convert.ToDouble(dt.Compute("SUM(Item_vaue)", string.Empty));
                    tds = Convert.ToDouble(dt.Compute("SUM(tds)", string.Empty));
                    adat = Convert.ToDouble(dt.Compute("SUM(adat)", string.Empty));

                    lblpurchasevalue.Text = purchaseval.ToString();
