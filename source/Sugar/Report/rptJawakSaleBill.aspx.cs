...
public partial class Report_rptJawakSaleBill : System.Web.UI.Page
{
    ...
    private void BindList()
    {
        try
        {
            ...
            {
                qry = "select top(1) doc_no,CONVERT(VARCHAR(10),doc_date,103) as doc_date, * from NT_1_JawakSaleBillPrint where doc_no=" + billno +
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        //string date = ds.Tables[0].Rows[i]["doc_date"];
                        //if (!string.IsNullOrWhiteSpace(ds.Tables[0].Rows[i]["doc_date"].ToString()))
                        //{
                            //string date = DateTime.Parse(dt.Rows[0]["doc_date"].ToString(), System.Globalization.CultureInfo.CreateSpecificCulture("en-GB")).ToString("dd/MM/yyyy");
                            ////ds.Tables[0].Rows[i]["doc_date"]=

                            //ds.Tables[0].Rows[i]["doc_date"] = date.ToString();
                        ...
                        {
                            ds.Tables[0].Rows[i]["Party_Gst"] = "" + ds.Tables[0].Rows[i]["Party_Gst"].ToString();
                        }
                        ...
                        if (!string.IsNullOrWhiteSpace(ds.Tables[0].Rows[i]["PartyFASSINO"].ToString()))
                        {
                            ds.Tables[0].Rows[i]["PartyFASSINO"] = "FSSAI NO: " + ds.Tables[0].Rows[i]["PartyFASSINO"].ToString();
                        }
                    }
    ...
    protected void dtlist_OnItemDataBound(object sender, DataListItemEventArgs e)
    {
        try
        {
            ...
            qry = "select * from NT_1_JawakSaleBillPrint where doc_no=" + billno + " and Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + 
