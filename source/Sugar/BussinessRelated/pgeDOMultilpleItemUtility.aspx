<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="pgeDOMultilpleItemUtility.aspx.cs" Inherits="Sugar_BussinessRelated_pgeDOMultilpleItemUtility" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function GetCustomers(pageIndex) {
            ...
            $.ajax({
                type: "POST",
                url: "../BussinessRelated/pgeDOMultilpleItemUtility.aspx/GetCustomers",
                data: '{searchTerm: "' + SearchTerm() + '", pageIndex: "' + pageIndex + '",Trantype: "' + $("#<%=drpTrnType.ClientID %>").val() + '",Trantype1: "' + $("#<%=drpdisp.ClientID %>").val() + '",PageSize: "' + $("#<%=drpPagesize.ClientID %>").val() + '",Company_Code: "' + '<%= Session["Company_Code"] %>' + '",year: "' + '<%= Session["year"] %>' + '"}',
        ...
        function OnSuccess(response) {
            ...
            if (customers.length > 0) {
                $.each(customers, function () {
                    ...
                    $("td", row).eq(10).html($(this).find("mill_rate").text());
                    $("td", row).eq(11).html($(this).find("sale_rate").text());
                    $("td", row).eq(12).html($(this).find("Tender_Commission").text());
                    $("td", row).eq(13).html($(this).find("desp_type").text());
                    $("td", row).eq(14).html($(this).find("truck_no").text());
                    $("td", row).eq(15).html($(this).find("SB_No").text());
                    $("td", row).eq(16).html($(this).find("EWay_Bill_No").text());
                    $("td", row).eq(17).html($(this).find("Delivery_Type").text());
                    $("td", row).eq(18).html($(this).find("transportshortname").text());
                    $("td", row).eq(19).html($(this).find("MM_Rate").text());
                    $("td", row).eq(20).html($(this).find("vasuli_rate").text());
                    $("td", row).eq(21).html($(this).find("doid").text());
            ...
            $('#gvCustomers  tr').each(function () {
                debugger;
                var number = $(this).children('td:eq(2)').text();
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('#gvCustomers ').on('dblclick', 'tr', function () {
                ...
                var Action = 1;
                var DO = tableData[21];
                var Tran_type = tableData[13];
                if (row_index > 0) {
                    if (isNaN(DO)) {
                    }
                    else {
                        window.open('../BussinessRelated/pgeDeliveryOrderMultipleItem.aspx?DO=' + DO + '&Action=' + Action);
                    }
                }
            });
    ...
    <asp:UpdatePanel ID="UpdatePanelMain" runat="server" UpdateMode="Conditional">
        <ContentTemplate>
            ...
            <asp:DropDownList ID="drpTrnType" runat="server" CssClass="ddl" Width="200px" Height="24px"
                ...
                <asp:ListItem Text="DeletedDO" Value="DD"></asp:ListItem>
            </asp:DropDownList>
            &nbsp; &nbsp; &nbsp; &nbsp;     
           <asp:DropDownList ID="drpdisp" runat="server" CssClass="ddl" Width="200px" Height="24px"
               AutoPostBack="false" onchange="functionToTriggerClick();">
               <asp:ListItem Text="All" Value="AI"></asp:ListItem>
                ...
                <center>
                    ...
                    <asp:GridView ID="gvCustomers" runat="server" AutoGenerateColumns="false" PageSize="10"
                        ...
                        <Columns>
                            ...
                            <asp:BoundField HeaderStyle-Width="10px" DataField="purc_order" HeaderText="T.Detail ID" />
                            ...
                            <asp:BoundField HeaderStyle-Width="170px" DataField="billtoshortname" HeaderText="SaleBill Name" />
                            <asp:BoundField HeaderStyle-Width="30px" DataField="salebillcityname" HeaderText="SBCity" />
                            <asp:BoundField HeaderStyle-Width="105px" DataField="shiptoshortname" HeaderText="ShipToName" />
                            ...
                            <asp:BoundField HeaderStyle-Width="10px" DataField="Tender_Commission" HeaderText="Cmisn" />
                            ...
                            <asp:BoundField HeaderStyle-Width="10px" DataField="Delivery_Type" HeaderText="Del. Type" />
                            <asp:BoundField HeaderStyle-Width="20px" DataField="transportshortname" HeaderText="Transport Name" />
                            <asp:BoundField HeaderStyle-Width="20px" DataField="MM_Rate" HeaderText="Memo" />
                            <asp:BoundField HeaderStyle-Width="20px" DataField="vasuli_rate" HeaderText="Vasuli" />
