        ...
        try
        {
            ...
            if (drpDOType.SelectedValue != "DO")
            {
                ...
                else
                {
                    ...
                    {
                        if (Convert.ToDouble(txtSaleTDS.Text) > 0)
                        {
                            double TotalBasicValue = 0.00;
                            double TDSAmount = 0.00;
                            double SaleTDSBalanceLimit = Convert.ToDouble(Session["BalanceLimit"].ToString());
                            TotalBasicValue = Convert.ToDouble(clsCommon.getString("select sum(TaxableAmount) as tb from nt_1_sugarsale where Ac_Code=" + txtSaleBillTo.Text + " and  Company_Code=" + Session["Company_Code"].ToString() + " and Year_Code=" + Session["Year"].ToString() + " and IsDeleted=1 "));

                            TotalBasicValue = TotalBasicValue + Convert.ToDouble(lblCashdiffvalue.Text);
                            if (TotalBasicValue >= SaleTDSBalanceLimit)
                            {
                                if (Convert.ToDouble(lblCashdiffvalue.Text) > SaleTDSBalanceLimit)
                                {
                                    TotalBasicValue = Convert.ToDouble(lblCashdiffvalue.Text) - SaleTDSBalanceLimit;
                                }
                                else
                                {
                                    TotalBasicValue = Convert.ToDouble(lblCashdiffvalue.Text);
                                }
                            }

                            if (TotalBasicValue > 0)
                            {
                                TDSAmount = Math.Round((Convert.ToDouble(lblCashdiffvalue.Text) * Convert.ToDouble(txtSaleTDS.Text) / 100), 2);
                            ...
                            hdnfTDSValue.Value = TDSAmount.ToString();
                        }

                        else
                        {
                            hdnfTDSValue.Value = "0.00";
        ...
        if (btnSave.Text == "Save" || hdnfsalebillto.Value != txtSaleBillTo.Text)
        {
            ...
            if (Amount >= balancelimit)
            {
                ...
                else if (SaleTDS == "U")
                {
                    ...
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), " ", "alert('Unregistered Person,Limit Exceeded over sale Limit !')", true);
                    btnSave.Enabled = false;
