...
public partial class Sugar_Master_PgeReceiptPaymentUtility : System.Web.UI.Page
{
    ...
    public static string GetCustomers(string searchTerm, int pageIndex, string Trantype, int PageSize, int Company_Code, int year)
    {
        ...
        foreach (var s in split)
        {
            ...
            name += "( nt_1_accountmaster_1.Short_Name like '%" + aa + "%' or narration like '%" + aa + "%' or dbo.nt_1_transacthead.doc_date like '%" + aa +
                "%' or amount like '%" + aa + "%' or dbo.nt_1_transacthead.tranid like '%" + aa + "%') and";
       ...
       //" and Year_Code=" + year + " order by doc_no desc,doc_date desc";
        ...
        string query = "SELECT ROW_NUMBER() OVER ( order by dbo.nt_1_transacthead.doc_no ASC) AS RowNumber,dbo.nt_1_transacthead.doc_no, dbo.nt_1_transacthead.tran_type, " +
                       "CONVERT(varchar(10), dbo.nt_1_transacthead.doc_date, 103) AS doc_date, nt_1_accountmaster_1.Short_Name AS Ac_Name_E, dbo.nt_1_transactdetail.amount, " +
                       "dbo.nt_1_transactdetail.credit_ac, dbo.nt_1_accountmaster.Short_Name AS Shortcreditname, dbo.nt_1_transactdetail.narration, " +
                       "dbo.nt_1_transacthead.tranid FROM         dbo.nt_1_accountmaster RIGHT OUTER JOIN " +
                       "dbo.nt_1_transactdetail ON dbo.nt_1_accountmaster.accoid = dbo.nt_1_transactdetail.ca RIGHT OUTER JOIN " +
                       "dbo.nt_1_transacthead LEFT OUTER JOIN " +
                       "dbo.nt_1_accountmaster AS nt_1_accountmaster_1 ON dbo.nt_1_transacthead.cb = nt_1_accountmaster_1.accoid ON " +
                       "dbo.nt_1_transactdetail.tranid = dbo.nt_1_transacthead.tranid " +
                       "where " + name + " and dbo.nt_1_transacthead.tran_type='" + Trantype + "' and dbo.nt_1_transacthead.Company_Code=" + Company_Code + " and dbo.nt_1_transacthead.Year_Code=" + year + " order by doc_no desc,doc_date desc";
