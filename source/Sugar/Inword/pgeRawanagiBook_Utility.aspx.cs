...
public partial class Sugar_Inword_pgeRawanagiBook_Utility : System.Web.UI.Page
{
    ...
    public static string GetCustomers(string searchTerm, int pageIndex, int PageSize, int Company_Code)
    {
        ...
        foreach (var s in split)
        {
            ...
            name += "(Doc_Date like '%" + aa + "%' or Remark like'%" + aa + "%') and";


        }
        name = name.Remove(name.Length - 3);

        string query = "SELECT ROW_NUMBER() OVER ( order by Doc_Date ASC) AS RowNumber,convert(varchar(10), Doc_Date,103) as Doc_Date,Remark "
       + " FROM rawangiline_head   where Company_Code=" + Company_Code + " and " + name + " order by Doc_Date desc ";
