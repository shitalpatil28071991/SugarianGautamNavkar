...
public partial class Sugar_Master_pgeAccountsmasterdeclearation : System.Web.UI.Page
{
    #region data section
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (isAuthenticate == "1" || User_Type == "A")
                {
                    ...
                    else
                    {
                        ...
                        this.NextNumber();
                        setFocusControl(txtcustomer_code);
                    }
    ...
    private void makeEmptyForm(string dAction)
    {
        try
        {
            if (dAction == "N")
            {
                ...
                btntxtcustomer_code.Enabled = false;
            ...
            if (dAction == "A")
            {
                ...
                btntxtcustomer_code.Enabled = true; 
            ...
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        hdnflvid.Value = dt.Rows[0]["id"].ToString();
                        hdnf.Value = dt.Rows[0]["id"].ToString();
                        txtDOC_DATE.Text = dt.Rows[0]["doc_dateConverted"].ToString();
                        txtcustomer_code.Text = dt.Rows[0]["ac_code"].ToString();
                        lblcustomer.Text = dt.Rows[0]["ac_Name"].ToString();  
                        txtGst_No.Text = dt.Rows[0]["amount"].ToString();
                        txttdstcslink.Text = dt.Rows[0]["narration"].ToString();
    ...
    #region [txtcustomer_code_TextChanged]
    protected void txtcustomer_code_TextChanged(object sender, EventArgs e)
    {
        strTextBox = "txtcustomer_code";
        searchString = txtcustomer_code.Text;
        csCalculations();
    }
    #endregion

    #region [btntxtcustomer_code_Click]
    protected void btntxtcustomer_code_Click(object sender, EventArgs e)
    {
        try
        {
            pnlPopup.Style["display"] = "block";
            hdnfClosePopup.Value = "txtcustomer_code";
            btnSearch_Click(sender, e);
        }
        catch
        {
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (hdnfClosePopup.Value == "txtcustomer_code")
            {

                lblPopupHead.Text = "--Select Party--";
                txtSearchText.Text = txtcustomer_code.Text;
    ...
    private void csCalculations()
    {
        try
        {
            if (strTextBox == "txtcustomer_code")
            {
                string partyName = string.Empty;
                if (txtcustomer_code.Text != string.Empty)
                {
                    searchString = txtcustomer_code.Text;
                    if (!clsCommon.isStringIsNumeric(searchString))
                    {
                        btntxtcustomer_code_Click(this, new EventArgs());
                    }
                    else
                    {
                        string qry = "";
                        hdnfacid.Value = clsCommon.getString("select accoid from qrymstaccountmaster where ac_code=" + txtcustomer_code.Text + " and Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + "");
                        qry = "select Ac_Name_E from " + qryAccountList + " where Ac_Code=" + txtcustomer_code.Text + " and Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString());
                        partyName = clsCommon.getString(qry);

                        if (partyName != string.Empty)
                        {
                            lblcustomer.Text = partyName;
                            setFocusControl(txtcustomer_code);
                        }
                        else
                        {
                            lblcustomer.Text = string.Empty;
                            txtcustomer_code.Text = string.Empty;
            ...
            {
                setFocusControl(txttdstcslink);
            }

            if (strTextBox == "txttdstcslink")
            {
