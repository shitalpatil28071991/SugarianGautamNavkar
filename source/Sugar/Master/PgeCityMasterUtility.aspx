<%@ Page Language="C#" Title="U-City Master" AutoEventWireup="true" MasterPageFile="~/MasterPage.master" CodeFile="PgeCityMasterUtility.aspx.cs" Inherits="Sugar_Master_PgeCityMasterUtility" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function GetCustomers(pageIndex) {
            ...
            $.ajax({
                type: "POST",
                url: "../Master/PgeCityMasterUtility.aspx/GetCustomers",
        ...
        function OnSuccess(response) {
            ...
            if (customers.length > 0) {
                ...
                $.each(customers, function () {
                    var customer = $(this);
                    $("td", row).eq(0).html($(this).find("city_code").text());
                    $("td", row).eq(1).html($(this).find("city_name_e").text());
                    $("td", row).eq(2).html($(this).find("pincode").text());
                    $("td", row).eq(3).html($(this).find("GstStateCode").text());
                    $("td", row).eq(4).html($(this).find("gststatename").text());
                    $("td", row).eq(5).html($(this).find("cityid").text());
    ...
    <script type="text/javascript">
        function Citymaster() {
            var Action = 2;
            var city_Code = 0;
            window.open("../Master/PgeCityMasterUtility.aspx", "_self")
            ...
            $('#gvCustomers').on('dblclick', 'tr', function () {
                ...
                if (row_index > 0) {
                    ...
                    if (isNaN(City_code)) {
                        ...
                        window.open('../Master/pgecityMaster.aspx?cityid=' + City_code + '&Action=' + Action);
                    }
           ...
           <asp:Label ID="Label3" runat="server" Text="Search:" Font-Bold="True"
               ForeColor="#CC3300" Font-Size="Medium"></asp:Label>
                ...
                <asp:GridView ID="gvCustomers" runat="server" AutoGenerateColumns="false" PageSize="10" Width="1500px" Height="60"
                    ...
                    <Columns>
                        <asp:BoundField HeaderStyle-Width="20px" DataField="city_code" HeaderText="City Code" />


                        <asp:BoundField HeaderStyle-Width="150px" DataField="city_name_e" HeaderText="City Name" AccessibleHeaderText="center" />
                        <asp:BoundField HeaderStyle-Width="50px" DataField="pincode" HeaderText="Pincode" />
                        <asp:BoundField HeaderStyle-Width="20px" DataField="GstStateCode" HeaderText=" GST State Code" />
                        <asp:BoundField HeaderStyle-Width="20px" DataField="gststatename" HeaderText="State Name" />
                        <asp:BoundField HeaderStyle-Width="50px" DataField="cityid" HeaderText="CityID" />
