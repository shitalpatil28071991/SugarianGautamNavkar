...
public partial class Sugar_Master_PgeSystemMasterUtility : System.Web.UI.Page
{
    ...
    private void BindDummyRow()
    {
        ...
        dummy.Columns.Add("System_Code");
        dummy.Columns.Add("System_Type");
        dummy.Columns.Add("System_Name_E");
        dummy.Columns.Add("System_Rate");
        dummy.Columns.Add("HSN");
        dummy.Columns.Add("systemid");
    ...
    public static string GetCustomers(string searchTerm, int pageIndex, string Trantype, int PageSize, int Company_Code)
    {
        ...
        foreach (var s in split)
        {
            ...
            name += "( System_Name_E like '%" + aa + "%' or System_Code like '%" + aa + "%' or HSN like '%" + aa + "%' or systemid like '%" + aa + "%' or System_Rate like '%" + aa + "%' ) and";
        ...
        string query = "SELECT ROW_NUMBER() OVER ( order by System_Code ASC) AS RowNumber,System_Code,System_Type,"
       + "System_Name_E ,System_Rate,HSN,systemid FROM nt_1_systemmaster   where Company_Code=" + Company_Code + " and " + name + " and System_Type='" + Trantype + "' order by System_Code desc";
        ...
        using (SqlConnection con = new SqlConnection(cs1))
        {
            using (SqlDataAdapter sda = new SqlDataAdapter())
            {
                ...
                using (DataSet ds = new DataSet())
                {
                    ...
                    DataRow[] results = ds.Tables[0].Select(f1 + f2, "System_Code desc");
                    if (results.Count() > 0)
                    {
