<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="PgeScriptMasterUtility.aspx.cs" Inherits="Sugar_Master_PgeScriptMasterUtility" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function GetCustomers(pageIndex) {
            ...
            $.ajax({
                type: "POST",
                url: "../Master/PgeScriptMasterUtility.aspx/GetCustomers",
        ...
        function OnSuccess(response) {
            ...
            if (customers.length > 0) {
                var a = $("td", row).eq(0).html($(this).find("FNO_Script_No").text());
                $.each(customers, function () {
                    var customer = $(this);
                    $("td", row).eq(0).html($(this).find("FNO_Script_No").text());
                    $("td", row).eq(1).html($(this).find("Script_Name").text());
                    $("td", row).eq(2).html($(this).find("FNO_Type").text());
                    $("td", row).eq(3).html($(this).find("Lot_Size").text());
                    $("td", row).eq(4).html($(this).find("Face_Value").text());
                    $("td", row).eq(5).html($(this).find("Script_Id").text());
    ...
    <script type="text/javascript">
        function PurcSale() {
            var Action = 2;
            var FNO_Script_No = 0;
            window.open("../Master/PgeScriptMasterUtility.aspx", "_self")
            window.open('../Master/PgeScriptMaster.aspx?FNO_Script_No=' + FNO_Script_No + '&Action=' + Action);
        }
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('#gvCustomers').on('dblclick', 'tr', function () {
                ...
                var Action = 1;
                var FNO_Script_No = tableData[0];

                if (row_index > 0) {

                    if (isNaN(FNO_Script_No)) {
           ...
           <asp:Label ID="Label3" runat="server" Text="Search:" Font-Bold="True"
               ForeColor="#CC3300" Font-Size="Medium"></asp:Label>
                ...
                <asp:GridView ID="gvCustomers" runat="server" AutoGenerateColumns="false" PageSize="10" Width="1500px" Height="60"
                    ...
                    <Columns>
                        <asp:BoundField HeaderStyle-Width="20px" DataField="FNO_Script_No" HeaderText="Script Code" />
                        <asp:BoundField HeaderStyle-Width="150px" DataField="Script_Name" HeaderText="Script_Name" AccessibleHeaderText="center" />
                        <asp:BoundField HeaderStyle-Width="20px" DataField="FNO_Type" HeaderText="Script Type" />
                        <asp:BoundField HeaderStyle-Width="70px" DataField="Lot_Size" HeaderText=" Lot Size" />
                        <asp:BoundField HeaderStyle-Width="70px" DataField="Face_Value" HeaderText="Face Value" />
                        <asp:BoundField HeaderStyle-Width="50px" DataField="Script_Id" HeaderText="Script Id" />
