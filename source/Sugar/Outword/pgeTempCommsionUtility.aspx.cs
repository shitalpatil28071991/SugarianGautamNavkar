...
public partial class Sugar_Outword_pgeTempCommsionUtility : System.Web.UI.Page
{
    ...
    public static string GetCustomers(string searchTerm, int pageIndex, int PageSize, int Company_Code, int year)
    {
        ...
        foreach (var s in split)
        {
            ...
            name += "( Ac_Name_E like '%" + aa + "%' or millname like '%" + aa + "%' or doc_no like '%" + aa + "%' or doc_dateConverted like '%" + aa + "%' or qntl like '%" + aa + "%' or texable_amount like '%" + aa + "%' or bill_amount like '%" + aa + "%' or commissionid like '%" + aa + "%'  ) and";
       ...
       + "Ac_Name_E ,millname,qntl,texable_amount,bill_amount,commissionid FROM qrycommissionbill   where " + name + " and Company_Code=" + Company_Code + " " +
       " and Year_Code=" + year + " and Tran_Type='LV' order by doc_no desc,doc_date desc ";
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
       ...
       + "Ac_Name_E ,millname,qntl,texable_amount,bill_amount,commissionid FROM " + qryCommon + " limit 15";
