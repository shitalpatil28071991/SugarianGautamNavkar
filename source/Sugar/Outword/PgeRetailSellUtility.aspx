<%@ Page Language="C#" Title="RetailSell Utility" AutoEventWireup="true" MasterPageFile="~/MasterPage.master" CodeFile="PgeRetailSellUtility.aspx.cs" Inherits="Sugar_Master_PgeRetailSellUtility" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function GetCustomers(pageIndex) {
            ...
            $.ajax({
                type: "POST",
                url: "../Outword/PgeRetailSellUtility.aspx/GetCustomers",
        ...
        function OnSuccess(response) {
            ...
            if (customers.length > 0) {
                $.each(customers, function () {
                    ...
                    $("td", row).eq(6).html($(this).find("KgPerKatta").text());
                    $("td", row).eq(7).html($(this).find("Delivered").text());
                    $("td", row).eq(8).html($(this).find("retailid").text());
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('#gvCustomers ').on('dblclick', 'tr', function () {
                ...
                var Action = 1;
                var retailid = tableData[8];
                var Tran_Type = tableData[1];
                if (row_index > 0) {
                    if (isNaN(retailid)) {
                    }
                    else {
                        window.open('../Outword/pgeSugarRetailForGSTnew.aspx?retailid=' + retailid + '&Action=' + Action + '&tran_type=' + Tran_Type);
                    }
                }
            });
