...
public partial class Sugar_Master_PgeRetailSellUtility : System.Web.UI.Page
{
    ...
    private void BindDummyRow()
    {
        ...
        dummy.Columns.Add("Party_Name");
        dummy.Columns.Add("Grand_Total");
        dummy.Columns.Add("millshortname");
        dummy.Columns.Add("KgPerKatta");
    ...
    public static string GetCustomers(string searchTerm, int pageIndex, string Trantype, int PageSize, int Company_Code, int year)
    {
        ...
        foreach (var s in split)
        {
            ...
            name += "( Party_Name like '%" + aa + "%' or doc_no like '%" + aa + "%' or doc_dateConverted like '%" + aa + "%' or Grand_Total like '%" + aa + "%' or retailid like '%" + aa + "%' ) and";
        ...
        string query = "SELECT ROW_NUMBER() OVER ( order by doc_no ASC) AS RowNumber,doc_no,doc_dateConverted as doc_date,Tran_Type,"
      + "Party_Name ,Grand_Total,millshortname,KgPerKatta,Delivered,retailid FROM qryretailsellheaddetail   where " + name + " and Tran_Type='" + Trantype + "' and Company_Code=" + Company_Code + " " +
