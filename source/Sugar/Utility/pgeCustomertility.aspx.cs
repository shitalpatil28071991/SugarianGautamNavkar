...
public partial class Sugar_Utility_pgeCustomertility : System.Web.UI.Page
{
    ...
    private void BindDummyRow()
    {
        ...
        dummy.Columns.Add("Client_Code");
        dummy.Columns.Add("Client_Name");
        dummy.Columns.Add("Pin_Code");
        dummy.Columns.Add("Gst_state_Code");
    ...
    public static string GetCustomers(string searchTerm, int pageIndex, int PageSize, int Company_Code)
    {
        ...
        foreach (var s in split)
        {
            ...
            name += "( Client_Name like '%" + aa + "%' or Client_Code like '%" + aa + "%' or Pin_Code like '%" + aa + "%' or Gst_state_Code like '%" + aa + "%' or accoid like '%" + aa + "%' ) and";


        }
        name = name.Remove(name.Length - 3);

        string query = "SELECT ROW_NUMBER() OVER ( order by Client_Code ASC) AS RowNumber,Client_Code,Client_Name,"
       + "Pin_Code ,Gst_state_Code,Gst_No,accoid FROM ClientRegistration   where accoid=0 and  " + name + " order by Client_Code desc ";
        ...
        using (SqlConnection con = new SqlConnection(cs1))
        {
            using (SqlDataAdapter sda = new SqlDataAdapter())
            {
                ...
                using (DataSet ds = new DataSet())
                {
                    ...
                    DataRow[] results = ds.Tables[0].Select(f1 + f2, "Client_Code desc");
                    if (results.Count() > 0)
                    {
