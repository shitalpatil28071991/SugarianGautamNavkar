...
public partial class Sugar_Utility_pgeEInoviceAndEwayBill : System.Web.UI.Page
{
            ...
            if (ID == string.Empty)
            {
                if (carporateSale == string.Empty)
                {
                    qryelement = "select doc_no as Doc_No,CONVERT(varchar,doc_date,103) as doc_date,UPPER(BuyerGst_No) as BuyerGst_No,UPPER(Buyer_Name) as Buyer_Name,UPPER(Buyer_Address) as Buyer_Address," +
                                      ...
                                      " CGSTAmount,SGSTAmount,IGSTAmount,TaxableAmount,isnull(CGSTRate,0) as CGSTRate,isnull(SGSTRate,0) as SGSTRate,isnull(IGSTRate,0) as IGSTRate,Distance,LORRYNO,System_Name_E,HSN,GSTRate,LESS_FRT_RATE,transportName,TransporterId from NT_1qryEInvoice  where  Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString())
            ...
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        string transportName = dt.Rows[0]["transportName"].ToString();
                        string TransporterId = dt.Rows[0]["TransporterId"].ToString();
                        if (transportName == "")
                        {
                            txtTransporter_Name.Text = "";
                        }
                        else {
                            txtTransporter_Name.Text = dt.Rows[0]["transportName"].ToString();
                        }
                        ...
                        {
                            txtTransporter_ID.Text = dt.Rows[0]["TransporterId"].ToString();
                        }
                        ...
                        if (txtBuyer_PIN.Text == txtDispatch_PincodeCode.Text)
                        {
    ...
    protected void btnGenEwayBill_Click(object sender, EventArgs e)
    {
        ...
        string Transname = txtTransporter_Name.Text;

        if (txtTransporter_ID.Text == "" || txtTransporter_ID.Text == "0")
        {
            Transid = "null";

        }
        else {
            Transid = "\"" + txtTransporter_ID.Text + "\"";
        }

        //if (txtTransporter_Name.Text == "")
        //{
        //    Transname = "Null";
        ...
        //else
        //{
        //    Transname = txtTransporter_Name.Text;
        //}
       ...
       // string Ewaybullno = string.Empty;
